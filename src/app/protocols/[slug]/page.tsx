...
import {
  PROTOCOL_FAMILIES,
  isProtocolFamilySlug,
  isProtocolFamilyAccessible,
} from '@/lib/protocolFamilies';
import { ProtocolFamilyDetailContent } from './ProtocolFamilyDetailContent';

export function generateStaticParams() {
  return PROTOCOL_FAMILIES.filter(isProtocolFamilyAccessible).map((f) => ({ slug: f.slug }));
}
...
export default async function ProtocolFamilyPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!isProtocolFamilyAccessible(family)) notFound();
