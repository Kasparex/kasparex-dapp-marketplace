...
function DashboardPageInner() {
  ...
  const setTab = (tab: typeof activeTab) => {
    setActiveTab(tab);
    const url = tab === 'overview' ? '/store/dashboard' : `/store/dashboard?tab=${tab}`;
  ...
  return (
    ...
    >
      <div className="mb-10">
        ...
        {state.address ? (
          <p className="text-sm text-zinc-500 font-mono">{state.address}</p>
        ) : null}
      </div>

      <HubWalletGateShell mode="replace" config={STORE_DASHBOARD_GATE}>
        <div className="flex items-center gap-1 p-1 k-control-group w-fit mb-8">
          <button
            type="button"
            onClick={() => setTab('overview')}
            className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === 'overview'
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === 'purchased'
              ? 'bg-zinc-100 dark:bg-zinc-800 text-[#02abb8] shadow-sm'
              : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'
              }`}
          >
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === 'products'
              ? 'bg-zinc-100 dark:bg-zinc-800 text-[#02abb8] shadow-sm'
              : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'
              }`}
          >
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-xl text-sm font-bold transition-all ${activeTab === 'sales'
              ? 'bg-zinc-100 dark:bg-zinc-800 text-[#02abb8] shadow-sm'
              : 'text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300'
              }`}
          >
        ...
        {isLoading ? (
          ...
          <div className="space-y-8">
            {(activeTab === 'overview' || activeTab === 'products' || activeTab === 'sales') && (
              ...
              <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden">
                <div className="p-6 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    {activeTab === 'overview' && (
                      <Link href="/store/dashboard?tab=products" className="text-xs font-bold text-[#02abb8] hover:underline uppercase tracking-wider">
                        View all
                      </Link>
                    )}
                    <Link href="/store/create" className="k-control-btn !border-cyan-500/30 !bg-cyan-500/10 !text-cyan-800 dark:!text-cyan-300 text-xs">
                      Add new
                    </Link>
                  </div>
                </div>
                ...
                {products.length === 0 ? (
                  <div className="p-12 text-center text-zinc-500">
                    ...
                    <Link href="/store/create" className="mt-4 inline-block px-4 py-2 bg-gradient-to-r from-cyan-600 to-teal-600 text-white rounded-xl text-sm font-bold uppercase tracking-wider hover:from-cyan-700 hover:to-teal-700 transition-colors">
                      ...
                      <div key={product.id} className="p-4 flex items-center gap-4 hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-3 text-xs text-zinc-500 mt-1">
                            ...
                            <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold uppercase ${product.status === 'active' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-zinc-100 text-zinc-500'}`}>
                              {product.status}
                            </span>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          {product.status === 'active' && (
                            <button type="button" onClick={() => handleArchive(product.id)} className="p-2 text-zinc-400 hover:text-red-500 transition-colors" title="Archive">
            ...
            {(activeTab === 'sales' || activeTab === 'overview') && stats.recentSales.length > 0 && (
              <div className="bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-2xl overflow-hidden">
                <div className="p-6 border-b border-zinc-200 dark:border-zinc-800 flex justify-between items-center">
                  ...
                  {activeTab === 'overview' && (
                    <Link href="/store/dashboard?tab=sales" className="text-xs font-bold text-[#02abb8] hover:underline uppercase tracking-wider">View all</Link>
                  )}
                </div>
                <div className="divide-y divide-zinc-100 dark:divide-zinc-800">
