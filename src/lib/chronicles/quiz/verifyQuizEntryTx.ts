...
import { CHRONICLE_QUIZ_ENTRY_FEE_KAS } from '@/lib/chronicles/quiz/constants';
...
/** Minimum treasury payment after max KREX/NFT listing discounts (~50% off base fee). */
function minQuizEntrySompi(): number {
  return Math.floor(kasToSompi(CHRONICLE_QUIZ_ENTRY_FEE_KAS) * 0.45);
}

export async function verifyChronicleQuizEntryTx(
  wallet: string,
  txHashRaw: string,
  ...
  const txHash = extractKaspaTransactionId(txHashRaw) ?? txHashRaw.trim().replace(/^0x/i, '').toLowerCase();
