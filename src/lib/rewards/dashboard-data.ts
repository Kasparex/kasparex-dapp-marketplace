...
export function getAllKREXTierRewards(): RewardItem[] {
  return Object.values(KREX_TIERS).map((tier) => ({
    ...
    feeReduction: undefined, // Fee is shown as percentage, not reduction
    points: undefined,
    benefits: [
      `${tier.multiplier}x reward multiplier`,
      `${tier.feePercent}% transaction fee`,
  ...
  return [
    {
      ...
      requirement: 'Tier 2+ KREX or Diamond NFT',
