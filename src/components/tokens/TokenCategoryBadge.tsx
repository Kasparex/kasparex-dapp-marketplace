'use client';

import { useRouter } from 'next/navigation';
import { KxListingCategoryChip } from '@/components/ui/KxListingCategoryChip';
import { getTokenCategory } from '@/lib/tokens/categories';
import type { Token } from '@/lib/tokens/types';

function CategoryIcon() {
  return (
    <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden>
      <path
        strokeLinecap="round"
        strokeLinejoin="round"
        d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z"
      />
    </svg>
  );
}

type TokenCategoryBadgeProps = {
  token: Token;
  className?: string;
  onFilter?: (category: string) => void;
};

export function TokenCategoryBadge({ token, className = '', onFilter }: TokenCategoryBadgeProps) {
  const router = useRouter();
  const category = getTokenCategory(token);

  return (
    <KxListingCategoryChip
      icon={<CategoryIcon />}
      className={className}
      title={`Filter by ${category}`}
      onClick={(e) => {
        e.preventDefault();
        e.stopPropagation();
        if (onFilter) {
          onFilter(category);
        } else {
          router.push(`/tokens?category=${encodeURIComponent(category)}`);
        }
      }}
    >
      {category}
    </KxListingCategoryChip>
  );
}
