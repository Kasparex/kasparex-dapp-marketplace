...
export function DAppDescriptionsPanel({ dapp }: DAppDescriptionsPanelProps) {
  ...
  if (!hasContent) {
    return (
      <p className="kx-body py-4">No description available for this dApp.</p>
    );
  }
  ...
  return (
    <div className="space-y-8">
      {dapp.description ? (
        <Section title="Description">{dapp.description}</Section>
      ) : null}
      {dapp.utility ? (
        <Section title="Utility">{dapp.utility}</Section>
      ) : null}
      {dapp.process ? (
        <Section title="How to use">{dapp.process}</Section>
      ) : null}
      {dapp.benefits ? (
        <Section title="Benefits">{dapp.benefits}</Section>
      ) : null}
      {dapp.security ? (
        <Section title="Security">{dapp.security}</Section>
      ) : null}
    </div>
  );
}
