...
export function CollectionCard({ collection, prefetched, tierLabel }: CollectionCardProps) {
  ...
  const imageUrl = collection.cardImageUrl ?? null;
  ...
  useEffect(() => {
    ...
    const loadSupply = async () => {
      try {
        ...
        if (!cancelled) setIsLoadingSupply(false);
      }
    };
      ...
      <KxListingCardBody className="flex flex-col flex-1">
        ...
        <div className="flex items-center justify-between mt-auto pt-2">
          {totalSupply !== null ? (
            ...
            <div className="text-sm text-zinc-400 dark:text-zinc-600">Loading supply…</div>
          ) : (
            <div className="text-sm text-zinc-400 dark:text-zinc-600">Supply unavailable</div>
          )}
