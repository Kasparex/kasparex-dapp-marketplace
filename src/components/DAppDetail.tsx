...
export function DAppDetail({ dapp }: DAppDetailProps) {
  ...
  useEffect(() => {
    ...
    if (!hasCheckedOnMount && !compatibility.isCompatible) {
  ...
  }, [compatibility.isCompatible, hasCheckedOnMount]);
  ...
  return (
    <div className="space-y-6">
      ...
      {!compatibility.isCompatible && !showCompatibilityModal && (
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-yellow-700 dark:text-yellow-400 mb-2">
                {compatibility.isKRC20Only
                  ? 'This dApp requires KRC-20 network (Kaspa-native wallet), which is not EVM-compatible. You will need a Kaspa-native wallet to use this dApp.'
                  : `This dApp requires ${compatibility.requiredChainNames.join(' or ')} network. You are currently on ${compatibility.currentChainName || 'an unsupported network'}.`}
              </p>
              <button
                onClick={() => setShowCompatibilityModal(true)}
                className="text-sm text-yellow-800 dark:text-yellow-300 hover:underline font-medium"
              >
                {compatibility.isKRC20Only ? 'Learn more about KRC-20 →' : 'Switch network →'}
              </button>
            </div>
          </div>
        </div>
