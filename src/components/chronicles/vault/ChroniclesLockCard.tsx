...
export function ChroniclesLockCard({
  ...
  if (overlay === 'none') {
    return (
      <div className="relative rounded-xl border border-cyan-500/30 chronicles-vault-card">
  ...
  return (
    <div className="relative rounded-xl border border-cyan-500/25 overflow-hidden min-h-[260px] chronicles-vault-card">
      {children != null ? (
        <div className="blur-sm opacity-35 pointer-events-none select-none max-h-[260px] overflow-hidden">{children}</div>
      ) : null}
      <div className="absolute inset-0 bg-gradient-to-b from-zinc-950/88 via-zinc-950/80 to-cyan-950/40 backdrop-blur-md flex flex-col items-center justify-center p-5 sm:p-7 text-center gap-2.5 overflow-y-auto">
        ...
        {description ? <p className="text-base text-zinc-300 dark:text-white/85 max-w-md leading-relaxed">{description}</p> : null}
        {priceLabel ? (
