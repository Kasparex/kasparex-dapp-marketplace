  ...
  return (
    <aside className="space-y-6 lg:sticky lg:top-6 self-start">
      {filtered.map((sec) => (
        ...
        >
          <h2 className="text-xs font-black uppercase tracking-widest text-[#02abb8] mb-3">{sec.title}</h2>
          {sec.body}
            ...
            <ul className="space-y-2.5">
              {sec.links.map((l) => (
                <li key={`${l.href}-${l.label}`}>
                  <Link
                    href={l.href}
                    className="text-sm font-semibold text-zinc-800 dark:text-zinc-200 hover:text-[#02abb8] transition-colors"
                  >
                    {l.label}
                  </Link>
                  {l.sublabel ? <span className="block text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">{l.sublabel}</span> : null}
                </li>
              ))}
            </ul>
          ) : null}
