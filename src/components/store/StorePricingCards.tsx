...
export function StorePricingCards({
  listingFee,
  updateFee,
  archiveFee,
  tier,
  ...
  return (
    <div className={`grid grid-cols-1 gap-4 md:grid-cols-3 ${className}`.trim()}>
      <StoreFeeCard
        ...
        tier={tier}
      />
      <StoreFeeCard title="Edit / update" feeKas={updateFee} tier={tier} />
      <StoreFeeCard title="Archive fee" feeKas={archiveFee} tier={tier} note="Paid when you archive a listing." />
    </div>
  );
}
