...
export function StoreSellerBenefitsPanel({
  className = '',
  variant = 'panel',
}: {
  className?: string;
  ...
  if (variant === 'compact') {
    const feePerk =
      discountPercent > 0 ? `${discountPercent}% off listing fees` : 'Hold 1M+ KREX for listing discounts';
    const pointsPerk =
    ...
    return (
      <>
        <aside
          ...
          aria-label="Seller perks. Hover for KREX tier details."
        >
        ...
        >
          ...
          <ul className="space-y-1 text-sm text-zinc-700 dark:text-zinc-300">
            <li>
              ...
              {discountPercent > 0
                ? `${discountPercent}% off Store listing fees (${tierLabel})`
                : `Stack 1M+ KREX for ${KREX_TIERS.Tier1.feeDiscountPercent}% off listing fees`}
            </li>
            <li>
              <span className={ui.accent}>•</span> List a product earns +{listPts} Hub Points at your tier
