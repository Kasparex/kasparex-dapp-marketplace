...
export function RevenueTreeFlowStory({ currentStep }: RevenueTreeFlowStoryProps) {
  ...
  const members: Member[] = [
    // Platform & Genesis Uplines (Above Mark)
    ...
    // The Central User (Mark)
    { id: 'mark', name: 'Mark (You)', level: 0, type: 'user', status: currentStep >= 1 ? 'active' : 'inactive', x: 50, y: 35 },

    // Downlines (Below Mark)
    // Level 1
    { id: 'chris', name: 'Chris', level: 1, type: 'downline', status: currentStep >= 2 ? 'active' : 'inactive', x: 35, y: 52, pct: 2 },
    { id: 'daniel', name: 'Daniel', level: 1, type: 'downline', status: currentStep >= 2 ? 'active' : 'inactive', x: 65, y: 52, pct: 2 },
    
    // Level 2
    { id: 'edward', name: 'Edward', level: 2, type: 'downline', status: currentStep >= 4 ? 'active' : 'inactive', x: 25, y: 64, pct: 5 },
    { id: 'frank', name: 'Frank', level: 2, type: 'downline', status: currentStep >= 4 ? 'active' : 'inactive', x: 45, y: 64, pct: 5 },
    
    // Level 3
    { id: 'george', name: 'George', level: 3, type: 'downline', status: currentStep >= 5 ? 'active' : 'inactive', x: 40, y: 74, pct: 10 },
    
    // Level 4
    { id: 'harry', name: 'Harry', level: 4, type: 'downline', status: currentStep >= 5 ? 'active' : 'inactive', x: 30, y: 82, pct: 20 },
  ...
  useEffect(() => {
    if (currentStep === 1) {
        triggerPulse('mark', 'u1', 'Link', '#02abb8');
      ...
      triggerPulse('chris', 'mark', `2%`, '#10b981');
      setTimeout(() => triggerPulse('mark', 'u1', `5%`, '#02abb8'), 200);
      setTimeout(() => triggerPulse('u1', 'u2', `10%`, '#10b981'), 400);
    } else if (currentStep === 5) {
      triggerPulse('edward', 'chris', `2%`, '#10b981');
      setTimeout(() => triggerPulse('chris', 'mark', `5%`, '#10b981'), 400);
    } else if (currentStep === 6) {
      const delay = 200;
      triggerPulse('henry', 'edward', `2%`, '#10b981');
      setTimeout(() => triggerPulse('edward', 'chris', `20%`, '#10b981'), delay * 2);
      setTimeout(() => triggerPulse('chris', 'mark', `45%`, '#10b981'), delay * 4);
    }
  }, [currentStep]);
  ...
  return (
    <div className="relative w-full h-[600px] bg-zinc-50 dark:bg-zinc-950 rounded-3xl border border-zinc-200 dark:border-zinc-800 overflow-hidden shadow-inner select-none">
      ...
      <div className="absolute top-6 left-8 z-10">
          <h4 className="text-[11px] font-bold text-zinc-400 uppercase tracking-[0.2em] mb-1">Live Flow Summary</h4>
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none">
          ...
          {pulses.map(pulse => {
              const fromM = members.find(m => m.id === pulse.from);
              const toM = members.find(m => m.id === pulse.to);
              if (!fromM || !toM) return null;
              return (
                  <g key={pulse.id}>
                    <circle r="4" fill={pulse.color || '#02abb8'} className="animate-pulse">
                        <animate attributeName="cx" from={`${fromM.x}%`} to={`${toM.x}%`} dur="0.8s" repeatCount="1" fill="freeze" />
                        <animate attributeName="cy" from={`${fromM.y}%`} to={`${toM.y}%`} dur="0.8s" repeatCount="1" fill="freeze" />
                    </circle>
                    <text fontSize="10" fontWeight="700" fill={pulse.color || '#02abb8'} textAnchor="middle" className="font-bold filter drop-shadow-sm">
                        <animate attributeName="x" from={`${fromM.x}%`} to={`${toM.x}%`} dur="0.8s" repeatCount="1" fill="freeze" />
                        <animate attributeName="y" from={`${fromM.y}%`} to={`${toM.y}%`} dur="0.8s" repeatCount="1" fill="freeze" />
                        {pulse.amount}
      ...
      {members.map(member => {
          ...
          const isUpline = member.type === 'upline';
              ...
              >
                  {hoveredMember === member.id && (
                      <div className="absolute -top-12 left-1/2 -translate-x-1/2 px-3 py-2 bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-2xl z-20 pointer-events-none whitespace-nowrap animate-in fade-in zoom-in-95 duration-200">
                          <p className="text-[10px] font-bold text-zinc-900 dark:text-white uppercase tracking-wider">{member.name}</p>
                          {member.address && <p className="text-[8px] font-mono text-zinc-400 mt-0.5">{member.address.slice(0, 6)}...{member.address.slice(-4)}</p>}
                          {member.pct && <p className="text-[9px] font-bold text-[#02abb8] mt-1">{member.pct}% Share</p>}
                      </div>
                  )}
                  ...
                  <div className={`mt-2 text-center transition-opacity duration-300 ${hoveredMember === member.id ? 'opacity-100' : 'opacity-60'}`}>
              ...
              <div className="bg-white dark:bg-zinc-950 rounded-3xl border border-zinc-200 dark:border-zinc-800 w-full max-w-sm overflow-hidden shadow-2xl scale-in-center" onClick={e => e.stopPropagation()}>
                  ...
                  <div className="p-6 space-y-4">
                      <p className="text-[11px] text-zinc-500 dark:text-zinc-400 leading-relaxed font-medium">
                          These wallets represent the platform&apos;s default upline.
                      </p>
                      <div className="space-y-1">
                          {[
                              ...
                              <div key={gen.name} onClick={() => copyToClipboard(gen.addr)} className="flex items-center justify-between p-2.5 rounded-xl bg-zinc-50 dark:bg-zinc-900 border border-zinc-100 dark:border-zinc-800 hover:border-[#02abb8]/50 transition-all cursor-pointer group">
                                  <div>
                  ...
                  <div className="p-4 bg-zinc-100 dark:bg-zinc-900 flex justify-center">
                      <button onClick={() => setIsGenesisModalOpen(false)} className="text-[10px] font-bold uppercase text-[#02abb8] tracking-[0.2em] animate-pulse">Close</button>
                  </div>
              </div>
          </div>
      )}
