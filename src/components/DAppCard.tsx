...
export function DAppCard({ dapp }: DAppCardProps) {
  ...
  return (
    <Link
      href={`/dapps/${slug}`}
      className="group block w-full text-left bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl overflow-hidden hover:shadow-xl hover:shadow-[#02abb8]/10 hover:border-[#02abb8]/30 hover:-translate-y-1 transition-all duration-300 relative flex flex-col min-h-[320px]"
    >
      <div className="p-5 flex flex-col flex-1 min-h-0">
        {/* Top Section: Profile Picture, Title, Network Badge, Dot Indicator, Favorite Star */}
        <div className="flex items-start gap-4 mb-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-2 mb-2">
              {/* Title */}
              <h3
                className="text-lg font-black text-zinc-900 dark:text-zinc-100 truncate group-hover:text-[#02abb8] transition-colors"
              ...
              <button
                ...
                className={`flex-shrink-0 p-1.5 rounded-lg transition-colors ${isFavoriteDapp
              ...
              >
                <svg className="w-5 h-5" fill={isFavoriteDapp ? 'currentColor' : 'none'} viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M11.049 2.927c.3-.921 1.603-.921 1.902 0l1.519 4.674a1 1 0 00.95.69h4.915c.969 0 1.371 1.24.588 1.81l-3.976 2.888a1 1 0 00-.363 1.118l1.518 4.674c.3.922-.755 1.688-1.538 1.118l-3.976-2.888a1 1 0 00-1.176 0l-3.976 2.888c-.783.57-1.838-.197-1.538-1.118l1.518-4.674a1 1 0 00-.363-1.118l-3.976-2.888c-.784-.57-.38-1.81.588-1.81h4.914a1 1 0 00.951-.69l1.519-4.674z" />
                </svg>
              </button>
            </div>

            {/* Network Badge and Dot Indicator Row */}
            <div className="flex items-center gap-2 flex-wrap mb-2">
              {/* Network Badge */}
              <span
                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border ${networkBadgeColor}`}
              ...
              {/* Dot Indicator */}
              <StatusIndicator dapp={mergedDApp} size="sm" clickable={false} className="flex-shrink-0" />
            </div>

            {/* Reward Token (if applicable) */}
            {tokenTicker && (
              <div className="flex items-center gap-1.5 mb-2">
                <span className="text-xs font-bold text-[#02abb8] uppercase tracking-wider">
                  {tokenTicker}
                </span>
        ...
        {/* Bottom Section: Category, Version, ID */}
        <div className="mt-auto pt-4 border-t border-zinc-100 dark:border-zinc-800">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {/* Category with Icon */}
              {category && (
                <div className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold uppercase tracking-wider rounded-lg border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-800/50 text-zinc-700 dark:text-zinc-300">
            ...
            <button
              ...
              className={`flex-shrink-0 p-1.5 rounded-lg transition-colors relative ${isLiked
