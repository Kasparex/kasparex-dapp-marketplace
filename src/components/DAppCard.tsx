...
export function DAppCard({ dapp, selectedNetwork = 'all' }: DAppCardProps) {
  ...
  const badgeKind = useMemo(() => {
    if (networkType === 'L1') {
      ...
      if (lower.includes('kaspa') && (lower.includes('testnet') || lower.includes('vprogs') || lower.includes('simulator')))
  ...
  return (
    ...
    >
      <KxListingCardMedia>
        {!(isKaspaConnected || isEvmConnected) ? (
          <div className="pointer-events-none absolute left-4 top-4 z-20 opacity-0 transition-opacity duration-200 group-hover:opacity-100">
            <span className={`inline-flex items-center gap-1 rounded-lg px-2.5 py-1 text-xs font-semibold shadow-sm ${topBadgeClassName}`}>
      ...
      <div
        className={`absolute inset-0 z-20 flex flex-col justify-between rounded-xl border border-zinc-900/10 bg-white dark:border-white/10 dark:bg-zinc-950 px-6 py-6 shadow-[inset_0_0_0_1px_rgba(0,0,0,0.14)] transition-opacity duration-200 ${
