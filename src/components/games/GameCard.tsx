...
export function GameCard({ game }: GameCardProps) {
  ...
  return (
    <>
      <KxListingCard
        href={access.isOpenable ? `/games/${game.slug}` : undefined}
        disabled={!access.isOpenable}
        onClick={!access.isOpenable ? handleCardClick : undefined}
        accent="games"
            ...
            <div className="flex items-center gap-2 text-sm">
              <span className="inline-flex items-center gap-2 text-zinc-900 dark:text-zinc-100 font-medium">
                ...
                {game.rewardConfig?.xpReward ? (
                  <span className="text-zinc-600 dark:text-zinc-400">
                    <span className="font-semibold text-zinc-700 dark:text-zinc-200">{game.rewardConfig.xpReward}</span> pts
                  </span>
                ) : null}
              </span>
            </div>
