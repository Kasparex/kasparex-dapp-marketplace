...
export function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    <KxListingCard
      href={project.route}
      accent={accent}
      className="w-full text-left relative flex flex-col min-h-[320px] hover:-translate-y-1"
    >
      <KxListingCardMedia aspectClass="h-40 aspect-auto">
        <div className="absolute inset-0 bg-gradient-to-br from-zinc-100 to-zinc-200 dark:from-zinc-800 dark:to-zinc-900 transition-all duration-700 group-hover:scale-[1.03]" />
        ...
        <div className="relative z-10 flex h-full w-full items-center justify-center opacity-50 group-hover:opacity-90 transition-all duration-700 transform group-hover:scale-110 group-hover:rotate-3">
          {project.category === 'Publishing' && (
            <svg className="w-20 h-20 text-zinc-700 dark:text-zinc-200 drop-shadow-2xl" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <KxListingCardBody comfortable className="relative z-10 flex-1 min-h-0">
        {getStatusBadge() ? (
          <div className="absolute top-5 right-5 z-10">{getStatusBadge()}</div>
        ) : null}
