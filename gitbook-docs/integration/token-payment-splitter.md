# Token Payment Splitter Integration Guide

Step-by-step guide for integrating Token Payment Splitter into your application.
...
```typescript
import { TOKEN_PAYMENT_SPLITTER_ABI } from '@/lib/contracts/abis';
```
...
function MyComponent() {
  const { data, isLoading } = useReadContract({
    ...
    abi: TOKEN_PAYMENT_SPLITTER_ABI,
...
export function useTokenPaymentSplitter() {
  // Read operations
...
Token Payment Splitter supports the following networks:
...
- [Contract Reference](./contracts/token-payment-splitter.md)
- [dApp Overview](./dapps/token-payment-splitter.md)
