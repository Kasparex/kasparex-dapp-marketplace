# Token Milestone Logger Integration Guide

Step-by-step guide for integrating Token Milestone Logger into your application.
...
```typescript
import { TOKEN_MILESTONE_LOGGER_ABI } from '@/lib/contracts/abis';
```
...
function MyComponent() {
  const { data, isLoading } = useReadContract({
    ...
    abi: TOKEN_MILESTONE_LOGGER_ABI,
...
export function useTokenMilestoneLogger() {
  // Read operations
...
Token Milestone Logger supports the following networks:
...
- [Contract Reference](./contracts/token-milestone-logger.md)
- [dApp Overview](./dapps/token-milestone-logger.md)
