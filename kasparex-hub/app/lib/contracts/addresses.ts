...
const getEnvVar = (key: string): string => {
  // In Remix, environment variables are available via process.env
  // For client-side, use ENV from loader data
  if (typeof window !== 'undefined') {
    // Client-side: try to get from window.ENV if available
